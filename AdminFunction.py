...
class adminfunc:
    ...
    def att(self, frame3):
        ...
        canvas = Canvas(frame3, width="5i", height="5i",
        ...
        def read():
            ...
            if self.en1 == "":
                lab3 = Label(
                    canvas, text="Enter the correct class", relief=GROOVE)
                lab3.place(x=10, y=0)
            elif self.en1 == "username":
                lab3 = Label(
                    canvas, text="Enter the correct class", relief=GROOVE)
    ...
    def execute1(self, canvas, myc2, en1, mydb2, frame3):
        ...
        for i in myc2:
            ...
            v[int(inc / 25) - 1] = Button(canvas, text="Present",
                                          command=lambda inc=inc: press(int(inc / 25) - 1), bg="green")
            v[int(inc / 25) - 1].place(x=70, y=inc)
            inc = inc + 25

        blast = Button(canvas, text="Submit",
                       command=lambda inc=inc: press(10))
    ...
    def attupd(self, a, clas, canvas, frame3):
        ...
        j = myc2.fetchall()
        myc1 = mydb1.cursor()
        myc1.execute("SELECT %s FROM attendance WHERE username='%s'" %
                     (str(clas), str(j[a][0])))
        k = myc1.fetchone()
        myc1.execute("UPDATE  attendance SET %s='%s' WHERE username='%s'" % (
            str(clas), str(int(k[0]) + 1), str(j[a][0])))
        mydb1.commit()
        if a == 10:
    ...
    def makeexe(self):
        ...
        myc1 = mydb1.cursor()
        myc1.execute("INSERT INTO login values('%s','%s')" %
                     (self.en03, self.en02))
        mydb2 = mysql.connector.connect(
            host="localhost",
            user="root",
        ...
        myc2 = mydb2.cursor()
        myc2.execute("INSERT INTO info values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
            self.en03, self.en03, self.en04, self.en05, self.en06, self.en07, self.en08, self.en09, self.en010, self.en011, self.en012))
        myc2.execute("INSERT INTO academics values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
            self.en03, '', '', '', '', '', '', '', '', ''))
        myc2.execute("INSERT INTO attendance values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
            self.en03, '0', '0', '0', '0', '0', '0', '0', '0', '0'))
        myc2.execute("INSERT INTO fee values('%s','%s','%s')" %
                     (self.en03, '0', '0'))
        mydb1.commit()
        mydb2.commit()
